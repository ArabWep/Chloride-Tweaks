...
echo Configuring Disk Cleanup...
for /f "skip=2 tokens=*" %%a in ('reg query "HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\Explorer\VolumeCaches"') do (
    reg add "%%a" /v StateFlags0001 /t REG_DWORD /d 2 /f >nul
)

:: Run Disk Cleanup
echo Running Disk Cleanup (this may take several minutes)...
start /wait cleanmgr /sagerun:1 >nul
...
for /d %%a in ("%USERPROFILE%\AppData\Local\Mozilla\Firefox\Profiles\*") do (
    ...
    rd /s /q "%%a\thumbnails" 2>nul
)

:: Thumbnail Cache
echo Cleaning thumbnail cache...
del /q /f /s "%USERPROFILE%\AppData\Local\Microsoft\Windows\Explorer\thumbcache_*.db" 2>nul
...
rd /s /q "%SystemRoot%\DeliveryOptimization" 2>nul
